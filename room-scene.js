...
import { createIsland } from './room-island.js?v=football-20260923';
import { createResident } from './room-resident.js?v=gait-20260923';
import { createPlayer } from './room-player.js?v=room-clear-20260923';
...
export async function startRoomScene({ stage, canvas, links, onSelect, onBrowse }) {
    ...
    const input = createWalkInput({
        ...
        canMove: () => !modalOpen && !browseOpen && !environment.open && !lost && !disposed,
    });
    ...
    function draw(time) {
        ...
        if (!modalOpen && (intent.active || walking || posed || changed || followed || moved || cutawayPending || settle > 0)) { settle = Math.max(0, settle - 1); requestFrame(); }
    ...
    function greet() {
        if (modalOpen || browseOpen || environment.open) return;
    ...
    function updateHint() {
        document.querySelector('.room-hint').firstChild.textContent = narrow.matches ? (stage.dataset.view === 'room' ? '左下摇杆移动 · 拖动 360° 环视 · 双指缩放' : '左下摇杆移动 · 双指缩放') : (stage.dataset.view === 'room' ? '拖动 360° 环视 · 滚轮缩放 · WASD 移动' : 'WASD / 方向键移动 · G 打招呼 · 拖动看四周');
    }
    ...
    function setView(indoors, relocate = false, clearInput = true) {
        ...
        updateHint();
        document.querySelector('.room-hint span').textContent = indoors ? '书桌右侧书架上的每个 App 都可以点开。' : '沿着小路走走，也可以走进小屋。';
